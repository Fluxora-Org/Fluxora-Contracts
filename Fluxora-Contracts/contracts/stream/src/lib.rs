...
/// Per-stream result for `create_streams_partial`.
#[contracttype]
#[derive(Clone, Debug)]
...
impl FluxoraStream {
    ...
    /// Create multiple payment streams with partial success (non-atomic batch).
    ///
    /// Unlike `create_streams`, this entrypoint processes each stream independently.
    /// If one stream fails (e.g., due to invalid parameters or insufficient balance),
    /// it does not roll back other successfully created streams in the same batch.
    ///
    /// # Parameters
    /// - `sender`: Address funding the streams
    /// - `streams`: Vector of `CreateStreamParams`
    ///
    /// # Returns
    /// - `Vec<CreateStreamResult>`: Results for each entry in the same order as input
    ///
    /// # Failure Isolation
    /// - Each stream creation is its own atomic unit (validation + transfer + persistence).
    /// - Failure of one entry does not affect others.
    /// - Events are emitted only for successfully created streams.
    ///
    /// # Authorization
    /// - Requires authorization from `sender`.
    pub fn create_streams_partial(
        ...
        for params in streams.iter() {
            // Internal closure to handle per-stream result
            let res = (|| -> Result<u64, ContractError> {
                Self::validate_stream_params(
                    &env,
                    &sender,
                    &params.recipient,
