...
| `create_streams_partial(sender, streams)` | Create multiple streams with per-entry failure isolation |
...
### create_streams_partial: Failure Isolation, Per-Entry Results, and Event Emission

`create_streams_partial(sender, streams)` provides a non-atomic batch creation path where individual failures do not roll back the entire batch.

#### Partial Batch Semantics

- **Single Auth**: `sender` must authorize once for the entire call.
- **Failure Isolation**: Each stream creation is an independent unit. If entry `i` fails (e.g., due to `InvalidParams` or `InsufficientBalance`), entries `0..i-1` (already processed) and `i+1..n` (yet to be processed) are unaffected.
- **Per-Entry Results**: Returns a `Vec<CreateStreamResult>` where each element contains:
  - `stream_id`: The ID of the created stream (0 if failed).
  - `success`: Boolean indicating if creation succeeded.
  - `error_code`: The `u32` discriminant of `ContractError` if failed.
- **Event Behavior**: A `StreamCreated` event is emitted **only** for successfully created streams. No event is emitted for failed entries.
- **Token Transfers**: Tokens are pulled from the `sender` via `pull_token` individually for each successful entry.
- **Ordering**: Results in the output vector correspond 1:1 with the input `streams` vector order.

#### Failure Isolation Rules

- **Validation Errors**: If `validate_stream_params` fails for an entry, that entry is marked as failed with the corresponding error code.
- **Transfer Errors**: If `pull_token` fails (e.g., insufficient balance or allowance), that entry is marked as failed.
- **Persistence Errors**: If state persistence fails, that entry is marked as failed.
- **Global Pause**: If the contract is globally paused (`is_creation_paused`), the entire call fails with `ContractError::ContractPaused` (checked once at the start).

#### Rationale

- **Treasury Efficiency**: Allows high-throughput operators to submit large batches without one invalid entry blocking the entire set.
- **Deterministic state**: Successfully created streams are persisted and events emitted immediately, ensuring on-chain and off-chain state stay in sync even for partial successes.
