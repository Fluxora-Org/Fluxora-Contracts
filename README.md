...
Documentation
Stream contract — Lifecycle, accrual formula, cliff/end_time, access control, events, and error codes.
Dust threshold — withdraw_dust_threshold formula, USDC examples, validation table, and template guidance.
Security — CEI ordering, token trust model, authorization paths, overflow protection.
Upgrade strategy — CONTRACT_VERSION policy, breaking-change classification, migration runbook.
Compile-time warnings — Warning gate, cleanup rules, and storage/gas/upgrade regression surface.
Deployment — Step-by-step testnet deployment checklist.
Recipient stream index — get_recipient_streams page cap, full-enumeration pattern, and DoS-prevention rationale.
Storage layout — Contract storage architecture, key design, and TTL policies.
Audit preparation — Entry-points and invariants for auditors.
Error codes — Full ContractError reference and the
FactoryError discriminant table
(factory decisions map to a stable u32 per variant; the CI guard is
contracts/factory/tests/factory_error_discriminants.rs).
Events — Emitted event shapes and topics.
Stream templates — Template lifecycle, auth, field mapping, and calldata savings.
What's in this repo
...
Tech stack
Rust (edition 2021)
soroban-sdk 21.7.7 (Stellar Soroban)
...
Update rust-toolchain.toml → run rustup update → rebuild and test
Update soroban-sdk version in Cargo.toml → update lock file → run full test suite
Verify compatibility with the target Stellar network (testnet, mainnet)
Document the change in the PR or release notes
Local setup
...
Stellar CLI (optional, for deploy/test on network)
...
See docs/DEPLOYMENT.md for the complete step-by-step deployment checklist.
...
Download the hash artifact from the CI run (GitHub Actions → Artifacts → fluxora_stream-wasm-hash).
Rebuild locally and verify against the committed reference:
Bash

bash script/verify-wasm-checksum.sh
Or verify existing artifacts without rebuilding:
Bash
...
See docs/security.md for the full reproducibility contract, auditor verification steps, and residual risks.
...
For deployment parameter schemas, factory variables, and initialization matrices, see docs/factory.md.
...
For technical consensus models, voter profiles, and cryptographic execution matrices, see docs/governance.md.
