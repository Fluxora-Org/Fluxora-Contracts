...
///
/// Bumped to 6: additive protocol-management surface (including sender rotation) was
/// deployed conservatively under the versioning policy.
///
/// Bumped to 7: added `DataKey::PausedStreamCount` and the permissionless
/// `get_paused_stream_count()` view for cheap protocol-wide pause monitoring.
...
pub enum DataKey {
    ...
    /// Protocol-wide count of streams currently stored in `StreamStatus::Paused`.
    ///
    /// ## Invariant
    /// On fresh deployments initialised with `CONTRACT_VERSION >= 7`, this value equals
    /// the number of persisted streams whose live status is exactly `Paused`.
    ///
    /// The counter is only adjusted on real status transitions into or out of `Paused`
    /// and is never allowed to go negative. Upgraded instances start with the key unset
    /// (`0`) until post-upgrade transitions repopulate it; see `docs/upgrade.md`.
    PausedStreamCount,
}
