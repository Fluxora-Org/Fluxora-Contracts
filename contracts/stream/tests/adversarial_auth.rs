...
impl<'a> Ctx<'a> {
    ...
    fn create_stream(&self) -> u64 {
        ...
        // Start at sequence 100 so pause/resume cooldown (17 ledgers) is never
        // triggered by freshly-created streams (last_pause_toggle_ledger == 0).
        self.env.ledger().set_sequence_number(100);
    ...
    fn pause_as_sender(&self, stream_id: u64) {
        // Advance sequence by 20 to clear the pause cooldown (MIN_PAUSE_INTERVAL_LEDGERS = 17).
        let seq = self.env.ledger().sequence();
        self.env.ledger().set_sequence_number(seq + 20);
        self.env.mock_auths(&[MockAuth {
            address: &self.sender,
            invoke: &MockAuthInvoke {
                contract: &self.contract_id,
