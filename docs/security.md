...
Calls save_stream to persist the new state.
Only then calls the token contract to transfer tokens to the recipient.
cancel_stream and cancel_stream_as_admin
After checks and computing the refund amount, the contract:

Sets stream.status = Cancelled and records cancelled_at.
Calls save_stream to persist the updated state.
Only then transfers the unstreamed refund to the sender.
...
Fee calculation: fee = (refund_amount × cancellation_fee_bps) / 10000 (truncated down)
Sender refund: refund_amount - fee
CRITICAL INVARIANT: The recipient's accrued amount is never affected by the fee.
...
Increases stream.deposit_amount with overflow protection.
Calls save_stream to persist the new deposit amount.
Only then calls the token contract to pull the top-up amount from the funder (pull_token).
...
Stream state (withdrawn_amount and status) is updated and saved.
The running contract_balance is decremented in memory.
Only then is the push_token external call made to transfer funds to the recipient (or specified destination).
...
Updates stream.end_time and stream.deposit_amount.
Calls save_stream.
Only then transfers the refund to the sender.
Emits end_shrt(stream_id) with StreamEndShortened { old_end_time, new_end_time, refund_amount }.
Failure path:
...
Security properties
Only admin can sweep: A non-admin caller is rejected with
ContractError::Unauthorized.
Excess-only transfer: saturating_sub ensures the transfer amount
is bounded by the actual excess; if liabilities exceed the balance, zero
is transferred.
CEI ordering: The excess is computed and the event is emitted before
the token transfer, consistent with the rest of the contract.
Reentrancy lock: acquire_reentrancy_lock / release_reentrancy_lock
protects the token transfer.
Zero-excess no-op: Calling when there is no excess returns 0 without
any token transfer or state change.
No recipient auth escalation: The recipient address is specified by the
...
fluxora_factory authorization model
for the exact factory-to-stream signing tree.

Cancellation-specific boundary checks:

Sender path (cancel_stream) cannot be executed by recipient or third party.
Admin path (cancel_stream_as_admin) cannot be executed by non-admin callers.
Streams in terminal states (Completed, Cancelled) are rejected with InvalidState.
Overflow protection
...
Creation Pause (set_creation_paused(true)): Causes create_stream and create_streams to fail with ContractError::ContractPaused. Existing streams are unaffected — withdrawals, cancellations, and other operations continue normally. This is stored under DataKey::CreationPaused.
Global Emergency Pause (set_contract_paused(true)): A "circuit breaker" that blocks all mutation operations across the entire protocol. This includes creation, withdrawals, cancellations, rate updates, and time adjustments. This is stored under DataKey::GlobalEmergencyPaused.
...
Only the actual stream recipient's key can authorize a delegated withdrawal.
A relayer cannot use an arbitrary self-generated key to burn the recipient's
nonce or trigger an unintended withdrawal.
Contract-account recipients (not ed25519 accounts) cannot use delegated_withdraw;
...
Checks: deadline → stream load → withdrawal frequency → nonce → public-key binding → signature.
Effects: update withdrawn_amount, optionally set Completed, call save_stream, increment nonce.
Interactions: push_token to stream.recipient, emit withdrew event (and optionally completed event).
...
Build WASM with pinned toolchain (cargo build --release --target wasm32-unknown-unknown)
Run bash script/verify-wasm-checksum.sh --no-build — compares each artifact against wasm/checksums.sha256
Fail with actionable error message if any checksum mismatches
Upload raw and optimized WASM + hash files as CI artifacts (30-day retention)
Local verification
...
Clone the repository at the commit tagged for audit.
Confirm rust-toolchain.toml channel matches the CI build.
Run bash script/verify-wasm-checksum.sh — all entries must print OK.
Compare the passing hash against the on-chain contract hash via stellar contract inspect.
Residual risks
...
All checks (stream exists, not Completed/Cancelled, time-terminal, destination set, not globally paused).
Compute withdrawable amount.
Update stream.withdrawn_amount and optionally set status = Completed.
Call save_stream to persist state.
Only then call push_token to transfer to destination.
...
#	Property	Coverage	Verification
6.1	batch_withdraw rejects duplicate stream IDs without changing state, balances, or liabilities	✅ Both	duplicate_id_batch_withdraw_rejected, docs/gas.md §Security Checklist Coverage Contract
6.2	batch_withdraw_to rejects duplicate stream IDs without paying any destination	✅ Both	duplicate_id_batch_withdraw_to_rejected, docs/gas.md §Security Checklist Coverage Contract
6.3	bulk_cancel_streams rejects duplicates atomically	✅ Both	duplicate_id_bulk_cancel_rejected_atomically, docs/gas.md §O(n²) duplicate-ID scan
6.4	bulk_resume_streams_as_admin rejects duplicates without changing status/counter	✅ Both	duplicate_id_bulk_resume_rejected_atomically, docs/gas.md §O(n²) duplicate-ID scan
