...
| 7 | `Stream` and `CreateStreamParams` gained optional `witness: Option<Address>` for off-chain compliance attestation cancellation (`witnessed_cancel_stream` entry-point added); `DataKey::SenderStreams(Address)` at discriminant 31; `DataKey::AutoRenewEnabled(u64)` at discriminant 29 for permissionless auto-renewal; `DataKey::PendingStreamOffer(u64)` at discriminant 32 and `DataKey::RecipientPendingOffers(Address)` at discriminant 33 for two-phase offer-then-accept stream creation; `create_stream_offer`, `accept_stream_offer`, `reject_stream_offer`, `cancel_stream_offer`, `get_stream_offer`, `get_recipient_pending_offers` entrypoints added; new `ContractError` variants `OfferNotFound` (38), `OfferExpired` (39), `OfferWrongRecipient` (40), `OfferWrongSender` (41); `Stream` and `CreateStreamParams` gained optional `irrevocable: Option<bool>` field blocking all cancel/shorten paths; `DataKey::PooledStreamShares(u64)` at discriminant 34, `DataKey::PooledStreamWithdrawn(u64, Address)` at discriminant 35 for pooled streams |
| 8 | Additive lookback-bounded creation, configuration, and claim calculation support added; `DataKey::MaxLookbackLedgers(u64)` at discriminant 30; `max_lookback_ledgers` config field, `set_max_lookback_ledgers` admin entry-point, and `create_streams_with_lookback` bounded-creation entry-point added without changing the persisted `Stream` shape |
| 9 | `delegated_withdraw` signature payload now accepts an optional `relayer_fee: i128` (recipient explicitly authorises it). The recipient receives `net_amount = gross_withdrawable - relayer_fee`, two sequential `push_token` calls are issued in CEI order (recipient first, relayer second), and the `BelowMinimumAmount` (16) precondition is now evaluated against `net_amount` (not the gross withdrawable). The `Withdrawal` event's `amount` field now publishes `net_amount` rather than the gross withdraw total — this is a **breaking event-payload change**. `CONTRACT_VERSION` bumped to 9 per the documented versioning policy |
